...
Cred = 'mongodb://'+USER+':'+PASS+'@ds051077.mongolab.com:51077/twitternet'

client = pymongo.MongoClient(Cred)
db = client['twitternet']
coll = db['TwitDump']
...
#   consumer_key,consumer_secret,skey,ssecret=privatebits.getTwitterKeys()
  consumer_key = os.getenv('TWITAK')
  consumer_secret =  os.getenv('TWITSEC')
  skey= os.getenv('TWOATOK')
  ssecret = os.getenv('TWOASEC')
...
def gephiOutputEdgeDefInner(api,f,members,typ='friends',maxf=4000):
  ...
  for id in members:
    ...
    rem = limits.items()[1][1]['friends']['/friends/ids']['remaining']
    print "Remaining Requests", rem
#     print "Request: ", i, ik
    print 50*'-'
    if rem == 0:
    	print "Wait for API limit"
    	sleep(5*60)
    	limits = api.rate_limit_status()
    	rem = limits.items()[1][1]['friends']['/friends/ids']['remaining']
    	print "Remaining: ", rem
...
#   g = openTimestampedFile(dirname,typ+fname)
#   File['rec'] = g.read()
  f.close()
  
  print("Waiting before uploading file")
  sleep(5)
  g = openReadFile(dirname,typ+fname)
#   File['rec'] = g.read()
  
  File[dirname]=g.read()
  
  report("...Gephi "+typ+" file generated")
#   f2=openTimestampedFile(dirname,typ+fname)
#   File['rec'] = f2.read()
  coll.insert(File)
...
def openReadFile(fpath,fname,timestamp=True):
  fpath='reports/'+fpath
  now = datetime.datetime.now()
  ts = now.strftime("_%Y-%m")
  checkDir(fpath)
  fpart=fname.split('.')
  if timestamp:
    f=open(fpath+'/'+fpart[0]+'%s.'%ts+fpart[1],'r+')
  else:
